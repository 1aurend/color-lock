...
At this point, the tutorial forks into two paths (kind of like a choose your own adventure??). If you continue with the instructions here, you'll build your lock using a class component, `this.state`, and the lifecycle method `componentDidUpdate`. If you `git checkout state-hooks`, you'll build your lock using a new React addition called [Hooks](https://reactjs.org/docs/hooks-intro.html), which allow us to use state variables and the logic of lifecycle methods in our functional components.  Both work: classes and binding `this` can be confusing; hooks are new and can't yet do absolutely everything classes can but should be more intuitive. (They also solve some other problems for more complex React applications, so they are pitched as the way of the future.) Take your pick.

Okay, so you're in for learning class components--great! A class is just a special type of function. For our purposes, it has two key features: (1) it can contain methods, other functions that instances of the class can call repeatedly, and (2) it uses `this` to allow us to keep track of state. Don't worry too much about how `this` works for purposes of this tutorial; the main point is that binding this allows us to keep a current value for our state variables as we render multiple components.

Enough of the conceptual stuff. How do we write a class? Well, first we need to do a little thinking in React. We need to introduce a class at the level in our component tree where the component needs to know state. Usually, this isn't bottom level components. These are typically "stateless". Note, this does not mean that they don't change how they look in response to inputs. We are already, for example, passing props to Button to allow it to render buttons that don't all look the same. But the question is does Button need to know more than that
